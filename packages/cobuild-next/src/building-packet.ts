...
export type WitnessLayoutTypes = keyof typeof WitnessLayoutFieldTags;
 ...
 * {@link MinWitnessLayoutFieldTag}, it will be parsed as WitnessLayout. Otherwise it is considered as
 * WitnessArgs.
 *
...
): "WitnessArgs" | WitnessLayoutTypes {
  ...
  if (buf.length > 4) {
    ...
    if (typeIndex >= MinWitnessLayoutFieldTag) {
      for (const [name, index] of Object.entries(WitnessLayoutFieldTags)) {
        if (index === typeIndex) {
          return name as WitnessLayoutTypes;
        }
      }
    } else {
      return "WitnessArgs";
...
export function tryParseWitness(
  ...
  | UnpackResult<typeof WitnessLayout>
  | { type: "WitnessArgs"; value: UnpackResult<typeof WitnessArgs> } {
  const buf = bytes.bytify(witness ?? []);
  if (buf.length > 4) {
    const typeIndex = Uint32LE.unpack(buf.slice(0, 4));
    try {
